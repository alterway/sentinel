...
from utils.test_utilities import SwarmService, SwarmNode
...
class TestSwarmServiceAdapter(unittest.TestCase):
    ...
    def test_get_services_on_manager(self, mock_get_services_object, mock_get_swarm_services, mock_is_manager):
    ...
    def test_get_services_on_worker(self, mock_get_services_object, mock_get_swarm_services, mock_is_manager):
        services = self.swarmservice_adapter.get_services()
        mock_is_manager.assert_called_once()
    ...
    @patch.object(SwarmServiceAdapter, '_get_service_exposed_ports', return_value=[{'internal_port': 80, 'external_port': 3000}])
    @patch.object(SwarmServiceAdapter, '_get_nodes_for_service', return_value=[
        Node(name='node1', address='192.168.50.4'),
        Node(name='node2', address='192.168.50.5'),
        Node(name='node3', address='192.168.50.6')
    ])
    @patch.object(SwarmServiceAdapter, '_get_swarm_service_labels_and_vars', return_value=[{'service_tags': 'http,https'}, ['service_name=hello']])
    ...
    @patch.object(SwarmServiceAdapter, '_get_service_exposed_ports', return_value=[])
    @patch.object(SwarmServiceAdapter, '_get_nodes_for_service', return_value=None)
    @patch.object(SwarmServiceAdapter, '_get_swarm_service_labels_and_vars', return_value=None)
    ...
    def test_get_nodes_for_service(self, mock_list_nodes):
        self.assertEqual(3, len(self.swarmservice_adapter._get_nodes_for_service(SwarmService(id='r1neuke2qg59ivhdblg4dvi7h'))))
    ...
    def test_get_service_exposed_ports_empty(self):
        self.assertEqual(0, len(self.swarmservice_adapter._get_service_exposed_ports(SwarmService(id='r1neuke2qg59ivhdblg4dvi7h', attrs={'Endpoint': {}}))))
